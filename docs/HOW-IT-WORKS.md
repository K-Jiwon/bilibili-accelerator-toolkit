...
- 调试端口没有鉴权，本机任何程序都能连上并注入代码，而且它在客户端运行期间**一直开启**
  （只监听 `127.0.0.1`，退出客户端后关闭）。不用加速启动器时就不会带这个端口。
- 注入的脚本拥有该页面的登录态访问能力，只运行可信脚本
- 不要用于付费内容或 DRM 绕过

## 7. 实测结论与注意点

- **切换视频不会新开 target**：客户端是在同一个播放页 target 上原地换地址，
  文档也不重建。因此"常驻 attach + `addScriptToEvaluateOnNewDocument` +
  `executionContextCreated` 补注入"这套组合可以稳定覆盖，换视频后脚本仍然生效。
- **不要手动刷新播放页**：客户端播放页被刷新后不会自动续播，会停在黑屏。
  这是客户端自身的行为（不注入脚本也能复现），重启客户端即可恢复。
- **不要用 `waitForDebuggerOnStart` 暂停新窗口**：暂停渲染进程会让新开的播放窗口
  出现黑屏/卡住。本项目故意不暂停窗口，只做"注册脚本 + 立即求值"。
- **会话断开要允许重新 attach**：`Target.detachedFromTarget` 之后必须把 targetId
  从已附加集合里移除（并做周期性对账），否则该目标直到注入器重启都不会再被注入。
